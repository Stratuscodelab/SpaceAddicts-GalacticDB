...
while True:
    ...
    # look up a name
        ...
        # clear the screen
            ...
            # Trait lookup
        edition = input("Enter an edition number (1-5554): ")
        if edition.isdigit() and int(edition) in range(1, 5555):
            ...
            for nft in nfts:
                if nft["edition"] == int(edition):
                    ...
                    ipfs_link = nft["image"]  # get the image link for the NFT
            if attributes:
                ...
                print(f"\nNo attributes found for Edition {edition}\n")
                        
            print(f"\nType 'show' to view the NFT image at: {ipfs_link}\n")
            user_input = input("Type 'show' to view the image or any other key to continue: ")
            if user_input.lower() == "show":
                print(f"Opening {ipfs_link}")
                webbrowser.register('brave', None, webbrowser.BackgroundBrowser('/Applications/Brave Browser.app/Contents/MacOS/Brave Browser'))
                browser = webbrowser.get('brave')  # specify the browser here
                webbrowser.open(ipfs_link)
            else:
                print("Invalid input. Please enter a number between 1 and 5554.")
